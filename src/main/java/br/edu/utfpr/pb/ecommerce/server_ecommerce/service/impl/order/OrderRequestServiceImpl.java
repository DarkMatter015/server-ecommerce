...
public class OrderRequestServiceImpl extends CrudRequestServiceImpl<Order, OrderUpdateDTO, Long> implements IOrderRequestService {
    ...
    private final br.edu.utfpr.pb.ecommerce.server_ecommerce.repository.OrderStatusRepository orderStatusRepository;
    ...
    private Order create(OrderRequestDTO request, User user) {
        ...
        order.setUser(user);
        order.setStatus(orderStatusRepository.findByName("PENDING").orElseThrow(() -> new RuntimeException("Error: Order status is not found.")));
        return orderRepository.save(order);
    }
    ...
    public void processOrder(OrderEventDTO orderEventDTO) {
        ...
        try {
            order.setStatus(orderStatusRepository.findByName("PROCESSING").orElseThrow(() -> new RuntimeException("Error: Order status is not found.")));
        ...
        order.setStatus(orderStatusRepository.findByName("COMPLETED").orElseThrow(() -> new RuntimeException("Error: Order status is not found.")));
        orderRepository.save(order);
    } catch (Exception e) {
        order.setStatus(orderStatusRepository.findByName("FAILED").orElseThrow(() -> new RuntimeException("Error: Order status is not found.")));
        orderRepository.save(order);
